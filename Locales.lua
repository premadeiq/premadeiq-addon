...
local en = {
    ...
    ["OptForecastTooltip"]     = "Show a win-chance line in the players panel, worked out from both teams' history. Right about 72% of the time — a hint, not a verdict. Costs about 1 MB of memory and no measurable time.",
...
local ruRU = {
    ...
    ["OptForecastTooltip"]     = "Строка в панели игроков с шансами обеих команд, посчитанная по их прошлым боям. Верна примерно в 72% случаев — это подсказка, а не приговор. Занимает около 1 МБ памяти, на производительность не влияет.",
...
local deDE = {
    ...
    ["OptForecastTooltip"]     = "Eine Zeile im Spielerfenster mit den Chancen beider Teams, berechnet aus ihren früheren Schlachten. In etwa 72% der Fälle richtig — ein Hinweis, kein Urteil. Braucht rund 1 MB Speicher und kostet keine messbare Leistung.",
...
local frFR = {
    ...
    ["OptForecastTooltip"]     = "Une ligne dans le panneau des joueurs avec les chances des deux équipes, calculées d'après leurs batailles passées. Juste environ 72% du temps — une indication, pas un verdict. Environ 1 Mo de mémoire, aucun coût mesurable.",
...
local esES = {
    ...
    ["OptForecastTooltip"]     = "Una línea en el panel de jugadores con las probabilidades de ambos equipos, calculadas a partir de sus batallas anteriores. Acierta cerca del 72% de las veces: es una pista, no un veredicto. Ocupa alrededor de 1 MB y no afecta al rendimiento.",
...
local itIT = {
    ...
    ["OptForecastTooltip"]     = "Una riga nel pannello giocatori con le probabilità di entrambe le squadre, calcolate dalle loro battaglie precedenti. Corretta circa il 72% delle volte: un suggerimento, non un verdetto. Occupa circa 1 MB e non incide sulle prestazioni.",
